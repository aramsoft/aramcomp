...
public class FileTool {
		...
		try {
			...
			for (int i = 0; dirList != null && i < dirList.length; i++) {
				...
				if (targetDirPath.equals(dirList[i])) {
					// 중간에 발견하면 반복체크는 종료한다.(결과요청 갯수에 도달한 경우에 한해) - 이곳에서 종료되면 이후
					// 하위에서 체크할 필요가 없다.
	 ...
	 */
	public static ArrayList<String> getLastDirectoryForModifiedDate(String baseDirPath, String fromDate, String toDate) {
		...
		String[] subDirList;
		String subDirPath = "";
		ArrayList<String> childResult = null;
		ArrayList<String> result = new ArrayList<String>();
	...
	public static boolean copyFilesByExtnt(String source, String extnt, String target) throws Exception {
		...
		if (srcFile.exists() && srcFile.isDirectory()) {
			...
			ArrayList<String> list = getSubFilesByExtnt(fileArray, extnt);
	...
	public static boolean execCopyFile(File srcFile, File tarFile) throws Exception {
		...
		try {
			...
			// 예외상황에 따른 처리 추가함. -> 만약 tarFile 이 디렉토리명인 경우 디렉토리 밑으로 새로 파일을 생성해서
			// 복사한다.. like DOS
					...
					if (originalFileList.length > 0) {
						for (int i = 0; i < originalFileList.length; i++) {
							...
							if (subF.isFile()) {
								...
								result = copyFile(originalDirPath + FILE_SEPARATOR + originalFileList[i], targetDir.getAbsolutePath() + FILE_SEPARATOR
										+ originalFileList[i]);
							} else {
					...
					if (originalFileList.length > 0) {
						for (int i = 0; i < originalFileList.length; i++) {
							...
							if (subF.isFile()) {
								...
								// 하위목록이 디렉토리이면 복사를 재귀적으로 호출한다.
								// 하위목록에 해당하는 폴더에 대해서는 생성일자 검사를 하지 않는다.(현재 폴더가
								// 복사대상이면 현재폴더의 하위는 제외없이 복사함)
								result = copyDirectory(originalDirPath + "/" + originalFileList[i], targetDirPath1 + "/" + originalFileList[i]);
							}
						}
					} else {
