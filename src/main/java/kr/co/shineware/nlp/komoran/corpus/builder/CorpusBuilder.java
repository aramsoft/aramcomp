...
import kr.co.shineware.nlp.komoran.corpus.model.CorpusData;
...
public class CorpusBuilder {
    ...
    public void build() {
    	...
    	try {
            for(String line : training.getCopursData()) {
                if (line.length() == 0) {
                    continue;
                }
...
//			}
    ...
    private List<Pair<String, String>> convertJaso(List<Pair<String, String>> answerList) {
        ...
        for (Pair<String, String> pair : answerList) {
            resultList.add(new Pair<String, String>(unitParser.parse(pair.getFirst()), pair.getSecond()));
        }
        return resultList;
    }
