...
<tbody>
	...
	<tr class="link" onclick="javascript:fn_aram_choose('<c:out value="${result.clbId}"/>','<c:out value="${result.clbNm}"/>'); return false;">
 
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${clubVO.totalRecordCount - index + 1}"/>
...
function fn_aram_linkPage(pageNo) {
    ...
    varForm.action = "${pageContext.request.contextPath}/cop/clb/listClubPopup.do";
    varForm.submit();
}
...
function fn_aram_choose(clbId, clbNm) {
	if(window.opener.gArguments["trgetId"]) window.opener.gArguments["trgetId"].value = clbId;
	if(window.opener.gArguments["trgetNm"]) window.opener.gArguments["trgetNm"].value = clbNm;
	window.close();
}
