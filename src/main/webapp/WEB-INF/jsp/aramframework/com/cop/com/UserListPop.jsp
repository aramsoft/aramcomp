...
<tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(userInfVO.searchVO.pageIndex-1) * userInfVO.searchVO.recordPerPage}"/>
	...
	<tr class="link" onClick="javascript:fn_aram_choose('<c:out value="${result.uniqId}" />','<c:out value="${result.userNm}" />','<c:out value="${result.userId}" />'); return false;">
		...
		<c:set var="reverseIndex" value="${userInfVO.searchVO.totalRecordCount - index + 1}"/>
