...
<thead>
  	<tr>
    	...
    	<th scope="col"            >스크랩명</th>
    	<th scope="col" width="20%">등록자</th>
...
<tbody>
 	...
 	<c:set var="startIndex" value="${(scrapVO.searchVO.pageIndex-1) * scrapVO.searchVO.recordPerPage}"/>
 	<c:forEach var="result" items="${resultList}" varStatus="status">
  	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.scrapId}"/>'); return false;">

  		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${scrapVO.searchVO.totalRecordCount - index + 1}"/>
...
function fn_aram_detail(scrapId) {
    ...
    varForm.action = "${pageContext.request.contextPath}/cop/scp/detailScrap.do";
    varForm.submit();
}
...
function fn_aram_help() {
	var url = "http://www.egovframe.org/wiki/doku.php\?id=egovframework:com:v2:cop:" + encodeURIComponent("스크랩관리");	
	window.open(url, "도움말");
}
