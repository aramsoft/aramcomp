...
<tbody>
	...
	<tr>
 	...
 	<%-- 데이터를 화면에 출력해준다 --%>
  	<c:set var="startIndex" value="${(diaryManageVO.searchVO.pageIndex-1) * diaryManageVO.searchVO.recordPerPage}"/>
	<c:forEach items="${resultList}" var="result" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.diaryId}"/>'); return false;">
 
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${diaryManageVO.searchVO.totalRecordCount - index + 1}"/>
