...
<%
 /**
  * @Class Name  : MapMaterialList.jsp
  * @Description : 지식맵(유형별) 목록
...
<table class="table-list" summary="이 표는 지식맵(유형별) 정보를 제공하며, 순번, 조직명, 지식유형명, 지식URL, 분류일자 정보로 구성되어 있습니다 .">
...
<tbody>
	...
	<tr> 
 	...
 	<c:set var="startIndex" value="${(mapMaterialVO.searchVO.pageIndex-1) * mapMaterialVO.searchVO.recordPerPage}"/>
	<c:forEach items="${resultList}" var="result" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_detail('${result.knoTypeCd}'); return false;">

 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${mapMaterialVO.searchVO.totalRecordCount - index + 1}"/>
 ...
 * 상세정보화면 
 ******************************************************** */
function fn_aram_detail(knoTypeCd) {
    var varForm = document.getElementById("mapMaterialVO");
    varForm.knoTypeCd.value = knoTypeCd;
    varForm.action = "${pageContext.request.contextPath}/dam/map/mat/detailMapMaterial.do";
    varForm.submit();
}
...
function fn_aram_regist(){
    var varForm = document.getElementById("mapMaterialVO");
    varForm.knoTypeCd.value = "";
    varForm.action = "${pageContext.request.contextPath}/dam/map/mat/registMapMaterial.do";
    varForm.submit();
}
