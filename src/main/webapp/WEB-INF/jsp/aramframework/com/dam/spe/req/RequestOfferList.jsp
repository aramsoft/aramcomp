...
<%
 /**
  * @Class Name  : RequestOfferList.jsp
	...
	<div class="keyword_area">
    	<form:select path="searchVO.searchCondition" class="select" title="조회조건 선택">
			<form:option value="" label="--선택하세요--" />
	   		...
	   		<form:option value="KNWLDG_CN" label="지식내용" />			   
   		</form:select>
...
<!-- 목록  -->
<table class="table-list" summary="이 표는 지식 정보제공/정보요청  정보를 제공하며, 순번, 조직명, 지식유형명, 지식명, 등록자, 등록일자  정보로 구성되어 있습니다 .">
<caption>지식 정보제공/정보요청 목록</caption>
<thead>
  	<tr>  
	    <th scope="col" width="7%" >No.</th>
	    <th scope="col" width="10%">조직명</th>
	    <th scope="col" width="18%">지식유형명</th>
	    <th scope="col"            >지식명</th>
...
<tbody>
	...
	<%-- 데이터를 화면에 출력해준다 --%>
  	<c:set var="searchVO" value="${requestOfferVO.searchVO}"/>
	...
	<tr class="link" onclick="javascript:fn_aram_detail('${result.knoId}'); return false;">
		...
		<td class="lt_text3"><c:out value="${result.knoTypeNm}"/></td>
		<td class="lt_text3L">
			<c:forEach var="i" begin="1" end="${result.threadDepth}" step="1">
			<c:if test="${i != result.threadDepth}">&nbsp;&nbsp;&nbsp;</c:if>
			<c:if test="${i == result.threadDepth}">┗RE:&nbsp;</c:if>
			</c:forEach>
			<c:out value="${result.knoNm}"/>
		</td>
...
function fn_aram_detail(knoId){
    ...
    varForm.action = "${pageContext.request.contextPath}/dam/spe/req/detailRequestOffer.do";
    varForm.submit();
}
