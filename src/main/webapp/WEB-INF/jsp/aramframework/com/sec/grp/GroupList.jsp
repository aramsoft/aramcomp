...
<form:form commandName="groupVO" action="" method="post">
...
<thead>
 ...
 <tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(groupVO.searchVO.pageIndex-1) * groupVO.searchVO.recordPerPage}"/>
 	<c:forEach var="result" items="${resultList}" varStatus="status">
  	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.groupId}"/>'); return false;">

 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${groupVO.searchVO.totalRecordCount - index + 1}"/>
