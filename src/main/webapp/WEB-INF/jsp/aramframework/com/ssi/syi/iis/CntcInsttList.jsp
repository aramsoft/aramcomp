...
<div id="search_area">
	...
	<div class="keyword_area">
  		<form:select path="searchVO.searchCondition" class="select" title="검색조건선택">
	   		...
	   		<form:option value="INSTT_NM" label="기관명" />
   		</form:select>
...
<tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(cntcInsttVO.searchVO.pageIndex-1) * cntcInsttVO.searchVO.recordPerPage}"/>
	<c:forEach items="${resultList}" var="result" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.insttId}"/>'); return false;">
 
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${cntcInsttVO.searchVO.totalRecordCount - index + 1}"/>
		<td class="lt_text3"><c:out value="${reverseIndex}"/></td>

		<td class="lt_text3"><c:out value="${result.insttId}"/></td>
		<td class="lt_text3"><c:out value="${result.insttNm}"/></td>
	</tr>
	</c:forEach>
</tbody>
</table>
 ...
 * 상세화면 처리 함수
 ******************************************************** */
function fn_aram_detail(insttId){
    var varForm = document.getElementById("cntcInsttVO");
...
function fn_aram_regist(){
    var varForm = document.getElementById("cntcInsttVO");
	varForm.insttId.value = "";
	varForm.action = "${pageContext.request.contextPath}/ssi/syi/iis/registCntcInstt.do";
	varForm.submit();
}
...
function fn_aram_help() {
	var url = "http://www.egovframe.org/wiki/doku.php\?id=egovframework:com:v2:ssi:" + encodeURIComponent("연계기관관리");	
	window.open(url, "도움말");
}
