...
<%
 /**
  * @Class Name  : CntcMessageList.jsp
  * @Description : 연계메시지 목록
...
<div id="search_area">
	...
	<div class="keyword_area">
  		<form:select path="searchCondition" class="select" title="검색조건선택">
	   		...
	   		<form:option value="CNTC_MSSAGE_NM" label="연계메시지명" />
   		</form:select>
...
<thead>
	<tr>
		...
		<th scope="col" width="30%">연계메시지ID</th>
		<th scope="col"            >연계메시지명</th>
	</tr>
</thead>
<tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(cntcMessageVO.pageIndex-1) * cntcMessageVO.recordPerPage}"/>
	<c:forEach items="${resultList}" var="result" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.cntcMessageId}"/>'); return false;">
 
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${cntcMessageVO.totalRecordCount - index + 1}"/>
		<td class="lt_text3"><c:out value="${reverseIndex}"/></td>

		<td class="lt_text3"><c:out value="${result.cntcMessageId}"/></td>
		<td class="lt_text3"><c:out value="${result.cntcMessageNm}"/></td>
	</tr>
	</c:forEach>
</tbody>
</table>
 ...
 * 상세화면 처리 함수
 ******************************************************** */
function fn_aram_detail(cntcMessageId){
    var varForm = document.getElementById("cntcMessageVO");
...
function fn_aram_regist(){
    var varForm = document.getElementById("cntcMessageVO");
	varForm.cntcMessageId.value = "";
	varForm.action = "${pageContext.request.contextPath}/ssi/syi/ims/registCntcMessage.do";
	varForm.submit();
}
...
function fn_aram_help() {
	var url = "http://www.egovframe.org/wiki/doku.php\?id=egovframework:com:v2:ssi:" + encodeURIComponent("연계메시지관리");	
	window.open(url, "도움말");
}
