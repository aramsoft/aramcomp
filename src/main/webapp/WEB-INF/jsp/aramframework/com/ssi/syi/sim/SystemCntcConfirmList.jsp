...
<%
 /**
  * @Class Name  : SystemCntcConfirmList.jsp
  * @Description : 시스템연계 승인 목록
...
<tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(systemCntcVO.searchVO.pageIndex-1) * systemCntcVO.searchVO.recordPerPage}"/>
	<c:forEach items="${resultList}" var="result" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.cntcId}"/>'); return false;">
	
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${systemCntcVO.searchVO.totalRecordCount - index + 1}"/>
...
function fn_aram_detail(cntcId){
	...
	varForm.action  = "${pageContext.request.contextPath}/ssi/syi/sim/editSystemCntcConfirm.do";
	varForm.submit();
}
...
function fn_aram_help() {
	var url = "http://www.egovframe.org/wiki/doku.php\?id=egovframework:com:v2:ssi:%EC%8B%9C%EC%8A%A4%ED%85%9C%EC%97%B0%EA%B3%84%EA%B4%80%EB%A6%AC";	
	window.open(url, "도움말");
}
