...
<tbody>
    ...
    <tr> 
     ...
     <%-- 데이터를 화면에 출력해준다 --%>
 	<c:set var="startIndex" value="${(batchSchdulVO.searchVO.pageIndex-1) * batchSchdulVO.searchVO.recordPerPage}"/>
    <c:forEach items="${resultList}" var="result" varStatus="status">
    <tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.batchSchdulId}"/>'); return false;">
 
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${batchSchdulVO.searchVO.totalRecordCount - index + 1}"/>
