...
<%
 /**
  * @Class Name  : InsttCodeRecptnDetail.jsp
  * @Description : 기관코드수신 상세조회
...
<!-- 등록  폼 영역  -->
<form:form modelAttribute="insttCodeRecptnVO" action="" method="post">
...
<table class="table-detail">
	<c:if test="${insttCodeRecptnVO.insttCode == null}">
    <tr>
        <td class="lt_text3">
	...
	<c:if test="${insttCodeRecptnVO.insttCode != null}">
  	<tr>
    	...
    	<td>
    		<c:out value="${insttCodeRecptnVO.insttCode}"/>
    	</td>
  	</tr>
  	<tr>
    	<th>
    	...
    	<td>
    		<c:out value="${insttCodeRecptnVO.allInsttNm}"/>
    	</td>
  	</tr>
  	<tr>
    	<th>
    	...
    	<td>
    		<c:out value='${fn:substring(insttCodeRecptnVO.changede, 0,4)}-${fn:substring(insttCodeRecptnVO.changede, 4,6)}-${fn:substring(insttCodeRecptnVO.changede, 6,8)}'/>
    	</td>
  	</tr>
  	<tr>
    	<th>
    	...
    	<td>
    		<c:out value='${fn:substring(insttCodeRecptnVO.changeTime, 0,2)}:${fn:substring(insttCodeRecptnVO.changeTime, 2,4)}:${fn:substring(insttCodeRecptnVO.changeTime, 4,6)}'/>
    	</td>
  	</tr>
  	<tr>
    	<th>
...
<table class="table-list" summary="기관코드수신이력정보테이블">
<thead>
	<tr>
	    <th width="10%">순번</th>
	    <th width="10%">발생일자</th>
	    <th width="20%">최하위기관명</th>
	    <th width="10%">변경구분</th>
	    <th width="10%">처리구분</th>
	    <th width="10%">생성일자</th>
	    <th width="10%">변경일자</th>
...
<tbody>
	...
	<c:forEach items="${insttCodeRecptnList}" var="result" varStatus="status">
	<tr>
	    ...
	    <td class="lt_text"><c:out value="${result.lowestInsttNm}"/></td>
	    <td class="lt_text">
	    ...
	    <td class="lt_text3"><c:out value='${fn:substring(result.changede, 0,4)}-${fn:substring(result.changede, 4,6)}-${fn:substring(result.changede, 6,8)}'/></td>
	    <td class="lt_text3">
...
function fn_aram_list(){
    var varForm = document.getElementById("insttCodeRecptnVO");
    varForm.action = "${pageContext.request.contextPath}/sym/ccm/icr/listInsttCodeRecptn.do";
    varForm.submit();  
}
