...
<%
 /**
  * @Class Name  : InsttCodeRecptnList.jsp
  * @Description : 기관코드수신 목록
...
<div id="search_area">
	...
	<div class="keyword_area">
   		<form:select path="searchVO.searchCondition" class="select" title="검색조건구분">
	   		...
	   		<form:option value='ALL_INSTT_NM' label="기관명" />
      		</form:select>
...
<table class="table-list" summary="기관코드정보 목록테이블">
<thead>
	<tr>
	    <th scope="col" width="7%" >No.</th>
	    <th scope="col" width="20%">기관코드</th>
	    <th scope="col"            >기관명</th>
	</tr>
</thead>
<tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(insttCodeRecptnVO.searchVO.pageIndex-1) * insttCodeRecptnVO.searchVO.recordPerPage}"/>
	<c:forEach items="${resultList}" var="result" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.insttCode}"/>'); return false;">
 
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${insttCodeRecptnVO.searchVO.totalRecordCount - index + 1}"/>
		<td class="lt_text3"><c:out value="${reverseIndex}"/></td>

	    <td class="lt_text3"><c:out value="${result.insttCode}"/></td>
	    <td class="lt_text"><c:out value="${result.allInsttNm}"/></td>
	</tr>
	</c:forEach>
</tbody>
</table>
...
function fn_aram_detail(insttCode){
    ...
    varForm.action = "${pageContext.request.contextPath}/sym/ccm/icr/detailInsttCodeRecptn.do";
    varForm.submit();
}
