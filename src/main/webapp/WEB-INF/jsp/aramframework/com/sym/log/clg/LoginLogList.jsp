...
<form:form commandName="loginLogVO" action="" method="post">
...
<tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(loginLogVO.searchVO.pageIndex-1) * loginLogVO.searchVO.recordPerPage}"/>
	<c:forEach var="result" items="${resultList}" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.logId}"/>'); return false;">
	
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${loginLogVO.searchVO.totalRecordCount - index + 1}"/>
