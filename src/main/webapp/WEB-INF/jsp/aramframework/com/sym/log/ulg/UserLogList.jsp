...
<tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(userLogVO.searchVO.pageIndex-1) * userLogVO.searchVO.recordPerPage}"/>
	<c:forEach var="result" items="${resultList}" varStatus="status">
	<tr  class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.occrrncDe}"/>'); return false;">
	
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${userLogVO.searchVO.totalRecordCount - index + 1}"/>
