  	...
  	<tr>
    	...
    	<td width="80%">
        	...
        	<a href="#" target="_blank" onclick="javascript:searchFileNm(); return false;" style="selector-dummy:expression(this.hideFocus=false);"  title="새 창으로 이동">
        		<img src="${pageContext.request.contextPath}/images/aramframework/com/cmm/icon/search.gif" alt='(프로그램파일명 검색)' width="15" height="15" />(프로그램파일명 검색)
        	</a>
    	</td>
  	</tr>
  	<tr>
...
<!-- 검색조건 유지 -->
<input type="hidden" name="searchCondition" value="${progrmManageDtlVO.searchCondition}" />
<input type="hidden" name="searchKeyword"   value="${progrmManageDtlVO.searchKeyword}" />
<input type="hidden" name="pageIndex"       value="${progrmManageDtlVO.pageIndex}" />
<input type="hidden" name="recordPerPage"   value="${progrmManageDtlVO.recordPerPage}" />
<!-- 검색조건 유지 -->
</form:form>
