...
<tbody>
	<%-- 데이터를 없을때 화면에 메세지를 출력해준다 --%>
 	...
 	<c:set var="startIndex" value="${(progrmManageVO.searchVO.pageIndex-1) * progrmManageVO.searchVO.recordPerPage}"/>
 	<c:forEach var="result" items="${resultList}" varStatus="status">
  	<tr >
  	
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${progrmManageVO.searchVO.totalRecordCount - index + 1}"/>
