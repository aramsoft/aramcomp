...
<tbody>
    ...
    <%-- 데이터를 화면에 출력해준다 --%>
 	<c:set var="startIndex" value="${(backupOpertVO.searchVO.pageIndex-1) * backupOpertVO.searchVO.recordPerPage}"/>
    <c:forEach items="${resultList}" var="resultInfo" varStatus="status">
    <tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${resultInfo.backupOpertId}"/>'); return false;">
 
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${backupOpertVO.searchVO.totalRecordCount - index + 1}"/>
