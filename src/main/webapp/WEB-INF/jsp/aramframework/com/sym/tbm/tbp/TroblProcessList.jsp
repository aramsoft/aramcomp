...
<%
/**
 * @Class Name : TroblProcessList.jsp
 * @Description : 장애처리결과 목록
...
<div id="search_area">
	...
	<div class="keyword_area">
    	...
    	<form:input path="strTroblNm" size="20" maxlength="25" title="검색어 입력" onkeypress="press();"/> 
 		...
 		<label for="strProcessSttus">처리상태 : </label>
...
<tbody>
    ...
    <tr>
 	...
 	<c:set var="startIndex" value="${(troblProcessVO.searchVO.pageIndex-1) * troblProcessVO.searchVO.recordPerPage}"/>
	<c:forEach var="result" items="${resultList}" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_detail('${result.troblId}'); return false;">
	
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${troblProcessVO.searchVO.totalRecordCount - index + 1}"/>
...
function fn_aram_detail(troblId) {
    ...
    varForm.action = "${pageContext.request.contextPath}/sym/tbm/tbp/registTroblProcess.do";
    varForm.submit();   
}
