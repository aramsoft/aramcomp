...
<tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(loginPolicyVO.searchVO.pageIndex-1) * loginPolicyVO.searchVO.recordPerPage}"/>
 	<c:forEach var="result" items="${resultList}" varStatus="status">
  	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.emplyrId}"/>'); return false;">
  	
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${loginPolicyVO.searchVO.totalRecordCount - index + 1}"/>
