...
<%
/**
 * @Class Name : LoginScrinImageList.java
 * @Description : 로그인화면이미지 목록
...
<form:form commandName="loginScrinImageVO" action="" method="post">
...
<table class="table-list" summary="로그인화면이미지에 대한 목록을 제공한다.">
...
<tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(loginScrinImageVO.searchVO.pageIndex-1) * loginScrinImageVO.searchVO.recordPerPage}"/>
	<c:forEach var="result" items="${resultList}" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.imageId}"/>'); return false;">
 
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${loginScrinImageVO.searchVO.totalRecordCount - index + 1}"/>
...
function fn_aram_detail(imageId) {
    ...
    varForm.action = "${pageContext.request.contextPath}/uss/ion/lsi/editLoginScrinImage.do";
    varForm.submit();
}
...
function fn_aram_regist() {
    ...
    varForm.action = "${pageContext.request.contextPath}/uss/ion/lsi/registLoginScrinImage.do";
    varForm.submit();
}
...
function fn_aram_help() {
	var url = "http://www.egovframe.org/wiki/doku.php\?id=egovframework:com:v2:uss:" + encodeURIComponent("로그인화면이미지관리");	
	window.open(url, "도움말");
}
