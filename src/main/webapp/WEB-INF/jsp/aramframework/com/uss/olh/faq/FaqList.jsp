...
<table class="table-list" summary="FAQ에 대한 목록을 제공합니다.">
...
<tbody>      
 	...
 	<c:set var="startIndex" value="${(faqManageVO.searchVO.pageIndex-1) * faqManageVO.searchVO.recordPerPage}"/>
	<c:forEach items="${resultList}" var="result" varStatus="status">
  	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.faqId}"/>'); return false;">
 
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${faqManageVO.searchVO.totalRecordCount - index + 1}"/>
 ...
 * 상세화면 처리 함수
 ******************************************************** */
function fn_aram_detail(faqId) {		
    var varForm = document.getElementById("faqManageVO");
    varForm.faqId.value = faqId;	
    varForm.action = "${pageContext.request.contextPath}/uss/olh/faq/detailFaq.do";
  	varForm.submit();	
}
...
function fn_aram_regist(){
    var varForm = document.getElementById("faqManageVO");
    varForm.faqId.value = "";	
    varForm.action = "${pageContext.request.contextPath}/uss/olh/faq/registFaq.do";
    varForm.submit();	
}
...
function fn_aram_help() {
	var url = "http://www.egovframe.org/wiki/doku.php\?id=egovframework:com:v2:uss:" + encodeURIComponent("faq관리");	
	window.open(url, "도움말");
}
