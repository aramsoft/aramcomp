...
<%@ page import="aramframework.com.uss.umt.service.MberManageService"  %>
...
<input type="hidden" name ="nextUrlName" value="상담등록">
<input type="hidden" name ="nextUrl" value="${pageContext.request.contextPath}/uss/olp/cns/registCnslt.do">
...
<table class="table-list" summary="상담에 대한 목록을 제공합니다.">
...
<tbody>
 	...
 	<c:set var="startIndex" value="${(cnsltManageVO.searchVO.pageIndex-1) * cnsltManageVO.searchVO.recordPerPage}"/>
	<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
  	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${resultInfo.cnsltId}"/>'); return false;">
 
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${cnsltManageVO.searchVO.totalRecordCount - index + 1}"/>
...
function fn_aram_detail(cnsltId) {
	...
	varForm.action = "${pageContext.request.contextPath}/uss/olp/cns/detailCnslt.do";
//	varForm.action = "${pageContext.request.contextPath}/uss/olp/cns/updateCnsltInqireCo.do";
	varForm.submit();
}
...
function fn_aram_regist(){
	...
	var loginRealnm_url 	= "${pageContext.request.contextPath}/uss/olp/cns/LoginRealnmChoice.do";
	...
	// 실명확인 화면
	var	realnm_url 	    = "${pageContext.request.contextPath}/sec/rnc/confirmRlnm.do?nextUrlName=button.cnsltregist&nextUrl=C&ihidnum=";
...
function fn_aram_regist_next(){
    var varForm = document.getElementById("cnsltManageVO");
	varForm.cnsltId.value = "";
    varForm.action = "${pageContext.request.contextPath}/uss/olp/cns/registCnslt.do";
    varForm.submit();
}
