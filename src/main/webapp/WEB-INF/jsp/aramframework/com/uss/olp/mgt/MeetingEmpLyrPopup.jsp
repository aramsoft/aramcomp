...
<%
 /**
  * @Class Name : MeetingEmplyrPopup.jsp
...
<tbody>
	...
	<tr class="link" onclick="javascript:fn_aram_choose('${result.uniqId}', '${result.userNm}'); return false;">
		...
		<td class="lt_text3L">${result.emplyrId}</td>
		<td class="lt_text3L">${result.userNm}</td>
		<td class="lt_text3">${result.offmTelno}</td>
		<td class="lt_text3L">${result.houseAdres} ${result.detailAdres}</td>
	</tr>
	</c:forEach>
</tbody>
</table>
 ...
 * 선택 처리 함수
 ******************************************************** */
function fn_aram_choose(empId, empName){
	if( window.opener.gArguments["mnaerId"] ) window.opener.gArguments["mnaerId"].value = empId;
	if( window.opener.gArguments["mnaerNm"] ) window.opener.gArguments["mnaerNm"].value = empName;
	window.close();
}
