...
<%
 /**
  * @Class Name : QustnrRespondList.jsp
  * @Description : 응답자정보 목록
...
<div id="search_area">
	...
	<div class="keyword_area">
    	<form:select path="searchVO.searchCondition" title="조회조건 선택">
	   		...
	   		<form:option value="BRTH" label="생년월일" />			   
   		</form:select>
...
<table class="table-list" summary="이 표는 응답자정보를 제공하며, 순번, 설문지정보, 응답자명, 등록자, 등록일자 정보로 구성되어 있습니다 .">
<thead>
	<tr>
	    <th scope="col" width="7%" >No.</th>
	    <th scope="col"            >설문지정보(제목)</th>
	    <th scope="col" width="10%">응답자명</th>
...
<tbody>
	...
	<tr>
	 ...
	 <%-- 데이터를 화면에 출력해준다 --%>
 	<c:set var="startIndex" value="${(qustnrRespondManageVO.searchVO.pageIndex-1) * qustnrRespondManageVO.searchVO.recordPerPage}"/>
	<c:forEach items="${resultList}" var="result" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_detail('${result.qestnrRespondId}'); return false;">
	
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${qustnrRespondManageVO.searchVO.totalRecordCount - index + 1}"/>
		<td class="lt_text3"><c:out value="${reverseIndex}"/></td>

	    <td class="lt_text3L"><c:out value="${result.qestnrSj}"/></td>
 ...
 * 상세화면 처리 함수
 ******************************************************** */
function fn_aram_detail(qestnrRespondId){
    var varForm = document.getElementById("qustnrRespondManageVO");
    varForm.qestnrRespondId.value = qestnrRespondId;
    varForm.action = "${pageContext.request.contextPath}/uss/olp/qrm/detailQustnrRespond.do";
    varForm.submit();
}
...
function fn_aram_regist(){
    var varForm = document.getElementById("qustnrRespondManageVO");
    varForm.qestnrRespondId.value = "";
    varForm.action = "${pageContext.request.contextPath}/uss/olp/qrm/registQustnrRespond.do";
    varForm.submit();
}
...
function fn_aram_help() {
	var url = "http://www.egovframe.org/wiki/doku.php\?id=egovframework:com:v2:uss:" + encodeURIComponent("설문응답자관리");	
	window.open(url, "도움말");
}
