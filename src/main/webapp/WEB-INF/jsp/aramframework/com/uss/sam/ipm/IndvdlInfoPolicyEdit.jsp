...
<%
 /**
  * @Class Name : IndvdlInfoPolicyEdit.jsp
  * @Description : 개인정보보호정책 수정
...
<!--  등록  폼 영역  -->
<form:form commandName="indvdlInfoPolicyVO" action="" method="post" enctype="multipart/form-data">
<input type="hidden" name="curTrgetId" value="${curTrgetId}" />
<input type="hidden" name="curMenuNo" value="${curMenuNo}" />

<form:hidden path="indvdlInfoId" />

<table class="table-register" summary="개인정보보호정책 수정을 제공한다.">
  	<tr>
    	<th width="20%">
    	...
    	<td width="80%">
    		<form:input path="indvdlInfoNm" title="이용약관명" size="70" maxlength="70" />  
...
<!-- 검색조건 유지 -->
<input type="hidden" name="searchCondition" value="${indvdlInfoPolicyVO.searchCondition}" />
<input type="hidden" name="searchKeyword"   value="${indvdlInfoPolicyVO.searchKeyword}" />
<input type="hidden" name="pageIndex"       value="${indvdlInfoPolicyVO.pageIndex}" />
<input type="hidden" name="recordPerPage"   value="${indvdlInfoPolicyVO.recordPerPage}" />
<!-- 검색조건 유지 -->
</form:form>
...
function fn_aram_update(){
	...
	if(confirm("<spring:message code='common.update.msg'/>")){
		varForm.action =  "${pageContext.request.contextPath}/uss/sam/ipm/updateIndvdlInfoPolicy.do";
		varForm.submit();
	}
}
