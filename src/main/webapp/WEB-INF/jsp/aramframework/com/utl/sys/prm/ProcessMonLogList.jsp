...
<%
/**
 * @Class Name  : ProcessMonLogList.jsp
 * @Description : 프로세스모니터링 로그 목록
...
<div class="content_title">
	<h2>프로세스모니터링 로그 목록</h2>
</div>

<form:form commandName="processMonLogVO" action="" method="post">
...
<div id="search_area">
	<div class="button_area">
		...
		<span class="button"><a href="#" onclick="javascript:fn_aram_list_processMon(); return false;"><spring:message code="button.list" /></a></span>
	</div>
...
<table class="table-list" summary="이 표는 프로세스모니터링 로그 정보를 제공하며, 순번, 로그ID, 프로세스명, 상태, 생성일시 정보로 구성되어 있습니다 .">
<caption>프로세스모니터링로그 상세조회</caption>
<thead>
	<tr>
  		<th scope="col" width="10%">순번</th> 
	    <th scope="col" width="30%">로그ID</th>
	    <th scope="col" width="20%">프로세스명</th>
...
<tbody>
 	...
 	<c:set var="startIndex" value="${(processMonLogVO.searchVO.pageIndex-1) * processMonLogVO.searchVO.recordPerPage}"/>
 	<c:forEach var="result" items="${resultList}" varStatus="status">
  	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.processId}"/>', '<c:out value="${result.logId}"/>'); return false;">
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${processMonLogVO.searchVO.totalRecordCount - index + 1}"/>
...
window.onload = function() {
    ...
    varForm.searchBgnHour.value = '<c:out value="${processMonLogVO.searchBgnHour}"/>';
    varForm.searchEndHour.value = '<c:out value="${processMonLogVO.searchEndHour}"/>';
};
...
function fn_aram_detail(processId, logId) {
    var varForm = document.getElementById("processMonLogVO");
    varForm.processId.value = processId;
    varForm.logId.value = logId;
    varForm.action = "${pageContext.request.contextPath}/utl/sys/prm/detailProcessMonLog.do";
    varForm.submit();
}
 ...
 * 마스터목록조회 함수
 ******************************************************** */
function fn_aram_list_processMon(){
	location.href = "${pageContext.request.contextPath}/utl/sys/prm/listProcessMon.do";
}
