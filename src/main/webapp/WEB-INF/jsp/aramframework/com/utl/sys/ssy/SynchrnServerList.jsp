...
<tbody>
	...
	<tr>
 	...
 	<c:set var="startIndex" value="${(synchrnServerVO.pageIndex-1) * synchrnServerVO.recordPerPage}"/>
 	<c:forEach var="result" items="${resultList}" varStatus="status">
  	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.serverId}"/>'); return false;">
 	
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${synchrnServerVO.totalRecordCount - index + 1}"/>
