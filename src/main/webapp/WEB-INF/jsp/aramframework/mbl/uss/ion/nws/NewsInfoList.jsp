...
function initList() {
	//$.mobile.changePage( "#list", { transition: "slideup" } );
 	...
 	var url = "${pageContext.request.contextPath}/uss/ion/nws/listNewsInfoJson.mdo";
	$.getJSON(url, $('#newsManageVO').serialize(), function(json) {            

       	$("#searchCondition option[value='" + json.newsManageVO.searchCondition + "']").attr('selected', 'selected');
			...
			html += '</li>';
     	    ...
     	    for (var i = 0; i < json.NewsList.length; i++) {
				...
				var ntceDe = year + "-" + month  + "-" + date; 
           		...
           		html += '<li>';
           		html += "    <a href=\"${pageContext.request.contextPath}/uss/ion/nws/detailNewsInfo.mdo?newsId=" + newsInfo.newsId + " \">";
