...
<% 
/**
 * @Class Name : ChargerListPopup.jsp
 * @Description : 담당자 목록
...
<table class="table-list" summary="이 표는 담당자정보를 제공하며, 부서, 직위, 사번, 사원명  정보로 구성되어 있습니다.">
...
<tbody>
	...
	<tr>
  	...
  	<c:set var="searchVO" value="${baseVO}"/>
 	<c:set var="startIndex" value="${(searchVO.pageIndex-1) * searchVO.recordPerPage}"/>
	<c:forEach var="result" items="${resultList}" varStatus="status">
	<tr class="link" onclick="javascript:fn_aram_choose('<c:out value="${result.userId}"/>', '<c:out value="${result.emplNo}"/>', '<c:out value="${result.userNm}"/>', '<c:out value="${result.orgnztNm}"/>'); return false;">
...
function fnInit(){
	...
	//document.frm.title.value = opener[0]; 
...
function fn_aram_linkPage(pageNo) {
    ...
    varForm.action = "${pageContext.request.contextPath}/cop/smt/djm/listCharger.do";
    varForm.submit();	
}
...
function fn_aram_choose(userId, emplNo, userNm, orgnztNm) {
