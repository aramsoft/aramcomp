...
<%
 /**
  * @Class Name : DtaUseStatsList.jsp
  * @Description : 자료이용현황통계 목록
...
<form:form modelAttribute="dtaUseStatsVO" action="" method="post">
	...
	<div class="search_right">
		<span class="keyword_area" >
			...
			<a href="#" onclick="fn_aram_NormalCalendar(document.forms[0].pmFromDate, document.forms[0].pmFromDateView); return false;" style="selector-dummy:expression(this.hideFocus=false);">
				<img src="${pageContext.request.contextPath}/images/com/cmm/icon/bu_icon_carlendar.gif" alt="달력창팝업버튼이미지">
			</a>
			~
...
<CAPTION>자료이용현황통계</CAPTION>
<thead>
  	<tr>
    	<th scope="col" width="25%">게시판명</th>
    	<th scope="col" width="10%">글번호</th>
    	<th scope="col" width="30%">글제목</th>
    	<th scope="col" width="25%">첨부파일명</th>
    	<th scope="col" width="10%">다운횟수</th>
  	</tr>
</thead>
<tbody>
 	...
 	<c:forEach var="result" items="${resultList}" varStatus="status">
  	<tr class="link" onclick="javascript:fn_aram_detail('<c:out value="${result.bbsId}"/>', '<c:out value="${result.nttId}"/>', '<c:out value="${result.atchFileId}"/>', '<c:out value="${result.fileSn}"/>'); return false;">
    	<td class="lt_text3"><c:out value="${result.bbsNm}"/></td>
    	<td class="lt_text3"><c:out value="${result.nttId}"/></td>
    	<td class="lt_text3"><c:out value="${result.nttSj}"/></td>
    	<td class="lt_text3"><c:out value="${result.fileNm}"/></td>
    	<td class="lt_text3"><c:out value="${result.downCnt}"/></td>
  	</tr>
 	</c:forEach>
</tbody>
</table>
...
<div style="width:80%; margin-bottom:20px;">
   	<table class="table-list" border="0">
	 	<c:forEach items="${dtaUseStatsBarList}" var="resultInfo" varStatus="status">
		<tr>
		   	<td width="100" height="10" class="lt_text3">${resultInfo.grpRegDate}</td>
		   	<td width="350" height="10">
		  		<img src="${pageContext.request.contextPath}/images/com/cmm/left_bg.gif" width="<c:out value='${resultInfo.grpCnt * dtaUseStatsVO.maxUnit}' />" height="10" align="left" alt="">&nbsp;&nbsp;${resultInfo.grpCnt}&nbsp;개
			</td>
		</tr>
		</c:forEach>
...
function initDate() {
    ...
    var fromDate = '<c:out value="${dtaUseStatsVO.pmFromDate}"/>';
    var toDate = '<c:out value="${dtaUseStatsVO.pmToDate}"/>';
...
function fn_aram_detail(bbsId, nttId, atchFileId, fileSn) {
    var varForm = document.getElementById("dtaUseStatsVO");
    varForm.bbsId.value = bbsId;
    varForm.nttId.value = nttId;
    varForm.atchFileId.value = atchFileId;
    varForm.fileSn.value = fileSn;
    varForm.action = "${pageContext.request.contextPath}/sts/dst/detailDtaUseStats.do";
    varForm.submit();
}
...
function fn_aram_help() {
	var url = "http://www.egovframe.org/wiki/doku.php\?id=egovframework:com:v2:sts:" + encodeURIComponent("자료이용현황통계");	
	window.open(url, "도움말");
}
