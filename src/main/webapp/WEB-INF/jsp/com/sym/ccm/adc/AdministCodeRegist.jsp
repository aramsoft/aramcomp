...
<%
 /**
  * @Class Name  : AdministCodeRegist.jsp
  * @Description : 행정코드 등록
...
<!-- 등록  폼 영역  -->
<form:form modelAttribute="administCodeVO" method="post">
<input type="hidden" name="curTrgetId" value="${curTrgetId}" />
<input type="hidden" name="curMenuNo" value="${curMenuNo}" />

<table class="table-register" summary="행정구역구분, 행정구역코드, 행정구역명, 생성일자, 폐기일자, 상위행정구역코드, 사용여부를 입력하여 행정코드를 등록한다.">
<CAPTION>행정코드 등록</CAPTION>
  	<tr>
    	<th width="20%">
    	...
    	<td width="80%">
      		<form:select path="administZoneSe" onChange="javascript:fn_aram_select_administZone()" title="행정구역구분">
  	...
  	<tr>
    	<th>
    		<span class="required_icon"></span>
    		<label for="administZoneCode">행정구역코드</label>
    	</th>
    	<td>
      		<form:input  path="administZoneCode" size="10" maxlength="10" title="행정구역코드"/>
      		<form:errors path="administZoneCode" cssClass="error"/>
    	</td>
  	</tr>
  	<tr>
    	<th>
  	...
  	<tr>
    	...
    	<td>
			...
			<a href="#" onclick="fn_aram_get_upperAdministCode(); return false;" style="selector-dummy:expression(this.hideFocus=false);">
				<img src="${pageContext.request.contextPath}/images/com/cmm/icon/search.gif" alt="행정코드찾기">
			</a>
...
function fn_aram_insert(){
	...
	if (confirm("<spring:message code='common.regist.msg' />")) {
		varForm.action = "${pageContext.request.contextPath}/sym/ccm/adc/insertAdministCode.do";
		varForm.submit();
	}
}
...
function fn_aram_select_administZone() {
    var varForm = document.getElementById("administCodeVO");
    varForm.upperAdministZoneCode.value = "";
    varForm.upperAdministZoneNm.value   = "";
}
