...
<div id="search_area">
	<div class="search_left">
	 	<strong>전체 : ${eventCmpgnVO.totalRecordCount} 건</strong>	
	</div>
...
<table class="table-list" summary="번호, 행사유형, 행사내용, 행사시작일자, 행사종료일자, 외부인사정보 , 작성자명, 등록일자  목록입니다">
<thead>
	<tr>
	    ...
	    <th scope="col" width="12%">외부인사정보</th>
...
<tbody>
	...
	<tr>
 	...
 	<c:set var="searchVO" value="${eventCmpgnVO}"/>
 	<c:set var="startIndex" value="${(searchVO.pageIndex-1) * searchVO.recordPerPage}"/>
	<c:forEach items="${resultList}" var="result" varStatus="status">
	<tr>
 		<c:set var="index" value="${startIndex + status.count}"/>
		<c:set var="reverseIndex" value="${totalRecordCount - index + 1}"/>
	    ...
	    <td class="lt_text3">
			<span class="link">
			<a href="#" onclick="javascript:fn_aram_list_tnextrlHrInfo('<c:out value="${result.eventId}"/>'); return false;">
				보기
			</a>
			</span>
	    </td>
...
function fn_aram_detail(eventId){
    ...
    varForm.action = "${pageContext.request.contextPath}/uss/ion/ecc/detailEventCmpgn.do";
    varForm.submit();
}
...
function fn_aram_regist(){
    ...
    varForm.action = "${pageContext.request.contextPath}/uss/ion/ecc/registEventCmpgn.do";
    varForm.submit();
}
 ...
 ******************************************************** */
function fn_aram_list_tnextrlHrInfo(eventId){
	var varForm = document.getElementById("eventCmpgnVO");
	varForm.eventId.value = eventId;
