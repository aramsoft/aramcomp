 				...
 				<tr>
 					...
 					<td style="background-color:#E3E3E3;">
						<c:forEach items="${statisticsList}" var="statisticsInfo" varStatus="status3">
  						...
  						<img src="${pageContext.request.contextPath}/images/com/cmm/chart/chart${status2.count}.JPG" width="${statisticsInfo.pollIemPercent}px" height="8" alt="차트"> (${statisticsInfo.pollIemIdCnt})건
  						<c:set var="chartCheck" value="${chartCheck+1}" />
  						</c:if>
  						</c:forEach>
						<c:if test="${chartCheck eq '0'}">
		   				<img src="${pageContext.request.contextPath}/images/com/cmm/chart/chart${status2.count}.JPG" width="1 px" height="8" alt="차트"> (0)건
		   				</c:if>
	 				</td>
 				</tr>
  				<c:set var="chartCheck" value="${0}" />
