...
<c:set var="qustnrManageVO.qestnrSj" value="${qustnrManageVO.qestnrPurps}"/>
...
<table class="table-register"  summary="수정 을 제공한다">
  	<tr>
    	...
    	<td width="80%">
      		<form:input path="qestnrSj" size="73" cssClass="txInput" maxlength="250" title="설문제목 입력" />
  	...
  	<tr>
    	...
    	<td>
    		<form:textarea path="qestnrWritngGuidanceCn" rows="3" cols="20" cssClass="txArea" title="설문작성안내내용 입력"/>
    ...
    <tr>
		...
		<table>
			...
			<tr>
				<c:forEach items="${listQustnrTmplat}" var="resultQustnrTmplat" varStatus="status">
		  		<td height="10" align="center"><input type="radio" name="qestnrTmplatIds" value="${resultQustnrTmplat.qestnrTmplatId}" style="border:0px" <c:if test="${qustnrManageVO.qestnrTmplatId ==  resultQustnrTmplat.qestnrTmplatId}">checked</c:if> disabled>${resultQustnrTmplat.qestnrTmplatTy}</td>
				</c:forEach>
			</tr>
		</table>
    	</td>
...
function fn_aram_update(){
	...
	if(confirm("<spring:message code='common.update.msg'/>")){
	    varForm.action = "${pageContext.request.contextPath}/uss/olp/qmc/updateQustnr.do";;
	    varForm.submit();
	}
}
